...
镜像已发布到 GitLab Container Registry：`registry.gitlab.com/da1234cao/weread-shelf`（打 tag 时由 CI 自动构建）。
...
docker run -d \
  ...
  registry.gitlab.com/da1234cao/weread-shelf:latest
```

### 方式二：docker compose

新建 `docker-compose.yml`（仓库根目录已附带一份）：

```yaml
services:
  weread-shelf:
    image: registry.gitlab.com/da1234cao/weread-shelf:latest
